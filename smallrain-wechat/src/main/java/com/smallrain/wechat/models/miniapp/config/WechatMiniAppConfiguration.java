...
public class WechatMiniAppConfiguration {
  ...
  @Autowired
  public WechatMiniAppConfiguration(WechatMiniAppProperties properties) {
  ...
  @PostConstruct
  public void init() {
    List<WechatMiniAppProperties.Config> configs = this.properties.getConfigs();
    if (configs == null) {
      throw new RuntimeException("配置出文件问题啦，自己看看去！");
    }
  ...
  private WxMaMessageRouter newRouter(WxMaService service) {
    ...
    router.rule().handler(logHandler).next().rule().async(false).content("模板").handler(templateMsgHandler).end().rule()
        .async(false).content("文本").handler(textHandler).end().rule().async(false).content("图片").handler(picHandler)
        .end().rule().async(false).content("二维码").handler(qrcodeHandler).end();
    return router;
  }
  ...
  private final WxMaMessageHandler picHandler = (wxMessage, context, service, sessionManager) -> {
    try {
      ...
      service.getMsgService().sendKefuMsg(
          WxMaKefuMessage.newImageBuilder().mediaId(uploadResult.getMediaId()).toUser(wxMessage.getFromUser()).build());
